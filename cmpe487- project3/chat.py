...
def processMessage(message):
    ...
    message=message[1:-1]       #delete [ and ]
    words = message.split(", ",3)  #split by headers
    ...
    if words[2] == 'announce':
        ...
        if words[0] in CHATS:
            CHATS[words[0]] += '\n' + words[0] + " - " + datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S') + ": " + incomingMessage
        else:
            CHATS[words[0]] = '\n' + words[0] + " - " + datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S') + ": " + incomingMessage
...
b1 = threading.Thread(name='background', target=background)
b2 = threading.Thread(name='backgroundUdp', target=backgroundUdp)
