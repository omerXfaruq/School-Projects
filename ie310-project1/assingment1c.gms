...
TABLE G(i,j) Investment Cost for Ships ($1000)(Merged train for not feasible parts)
        1       2       3       4       5
    1   40      60      0       40      80
    2   60      40      80      20      40
    3   0       0       80      60      100
...
Variables
        x(i,j) amount of flow from plant i to customer j via rails
        y(i,j) amount of flow from plant i to customer j via ships
        z   objective;
...
constraint1(i).. sum(j,x(i,j)+y(i,j)) =l= a(i)*1000;
constraint2(j).. sum(i,x(i,j)+y(i,j)) =g= b(j)*1000;
cost.. sum(j,sum(i,x(i,j)*C(i,j) + y(i,j)*(D(i,j)+E(i,j)/20)))=e=z;

model assingment1c /all/;
solve assingment1c using lp minimizing  z;
display x.l,y.l,z.l;
